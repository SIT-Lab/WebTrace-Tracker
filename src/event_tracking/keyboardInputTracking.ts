...
export const onKeyboard = async (event: KeyboardEvent) => {
        ...
        switch (currentKeyboardInput.code) {
            ...
            case "Numpad9":
                if (isNaN(parseInt(currentKeyboardInput.key)) == true) { // Numpad keys를 눌렀지만 숫자가 아닌 다른 부호를 클릭한 경우 키보드 입력이 종료되었다고 간주
                    ...
                    keyboardLogEvent("control input", event, false, Date.now(), " - ", currentKeyboardInput.type, currentKeyboardInput.key, currentKeyboardInput.code);
                    endKeyboardInput(Date.now() + 1, `${currentKeyboardInput.key}`, true)
                    return;
                } else {
                    console.log("currentKeyboardEvent.key -> ", currentKeyboardInput.key)
                    keyboardLogEvent("data input", event, false, Date.now(), "input start", currentKeyboardInput.type, currentKeyboardInput.key, currentKeyboardInput.code);
                }
                break;
            case "Space":
            case " ":
            ...
            default:
                ...
                if ((currentKeyboardInput.key.length == 1) && (currentKeyboardInput.key.match(/[a-z]/) || currentKeyboardInput.key.match(/[A-Z]/) || currentKeyboardInput.key.match(/[~`!@#$%^&*()-_+={[}\]|\\:;"'<,>.?/]/))) {
                    console.log(`data input: ${currentKeyboardInput.key}`);
                    keyboardLogEvent("data input", event, false, Date.now(), "input start", currentKeyboardInput.type, currentKeyboardInput.key, currentKeyboardInput.code);
                } else {
                    //Start State에 입력한 키가 Non-printable Keys라면 시작과 동시에 종료
                    lastKeyboardInputTime = now
                    console.log(`control input: input end due to ${currentKeyboardInput.key}`);
                    // keyboardLogEvent("keyboard input", event, false, Date.now(), "input start", currentKeyboardInput.type, currentKeyboardInput.key, currentKeyboardInput.code);
                    // endKeyboardInput(Date.now() + 1, `${currentKeyboardInput.key}`)
                    keyboardLogEvent("control input", event, false, Date.now(), " - ", currentKeyboardInput.type, currentKeyboardInput.key, currentKeyboardInput.code);
                    endKeyboardInput(Date.now() + 1, `${currentKeyboardInput.key}`, true)
        ...
        switch (currentKeyboardInput.code) {
            ...
            case "Numpad9":
                if (isNaN(parseInt(currentKeyboardInput.key)) == true) { // Numpad keys를 눌렀지만 숫자가 아닌 다른 부호를 클릭한 경우 키보드 입력이 종료되었다고 간주
                    ...
                    endKeyboardInput(Date.now() + 1, `${currentKeyboardInput.key}`, false)

                    keyboardLogEvent("control input", event, false, Date.now() + 2, " - ", currentKeyboardInput.type, currentKeyboardInput.key, currentKeyboardInput.code);
                    endKeyboardInput(Date.now() + 3, `${currentKeyboardInput.key}`, true)
                    return;
                } else {
                    console.log("currentKeyboardEvent.key -> ", currentKeyboardInput.key)
                    keyboardLogEvent("data input", event, false, Date.now(), "input ongoing", currentKeyboardInput.type, currentKeyboardInput.key, currentKeyboardInput.code);
                }
                break;
            case "Space":
            case " ":
            ...
            case "NumpadEqual":
                // Printable 키 또는 Non-printable 키 중 일부(Space, Shift, CapsLock 등)를 입력한 경우
                console.log(`data input: input ongoing`);
                console.log("currentKeyboardEvent.key -> ", currentKeyboardInput.key)
                keyboardLogEvent("data input", event, false, Date.now(), "input ongoing", currentKeyboardInput.type, currentKeyboardInput.key, currentKeyboardInput.code);
                break;
            default:
                ...
                if ((currentKeyboardInput.key.length == 1) && (currentKeyboardInput.key.match(/[a-z]/) || currentKeyboardInput.key.match(/[A-Z]/) || currentKeyboardInput.key.match(/[~`!@#$%^&*()-_+={[}\]|\\:;"'<,>.?/]/))) {
                    ...
                    //Start State에 입력한 키가 Non-printable Keys라면 시작과 동시에 종료
                    lastKeyboardInputTime = now
                    console.log(`data input: input end due to ${currentKeyboardInput.key}`);
                    endKeyboardInput(Date.now() + 1, `${currentKeyboardInput.key}`, false)
    ...
    keyboardInputTimeout = window.setTimeout(() => {
        ...
        if (currentKeyboardInput !== null) {
            keyboardLogEvent("data input", currentKeyboardInput, true, Date.now(),
 ...
 */
export const endKeyboardInput = (time: any, isEscapeKey?: string, isControlKey?: boolean) => {
        ...
        if (isControlKey) {
            ...
            if (currentKeyboardInput !== null) {
                if (isEscapeKey) {
                    ...
                    keyboardLogEvent("data input", currentKeyboardInput, true, time,
                        "input end", //keyboardInputState
                        undefined, // keyboardInputType
                        undefined, // keyboardInputPressedKey
                        undefined // keyboardInputKeyCode
                    ...
                    console.log(`data input: input end due to other event`);
                    keyboardLogEvent("data input", currentKeyboardInput, true, time,
                        "input end", //keyboardInputState
                        undefined, // keyboardInputType
                        undefined, // keyboardInputPressedKey
